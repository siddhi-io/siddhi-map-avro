...
<p style="word-wrap: break-word">Avro to Event input mapper. Transports which accepts Avro messages can utilize this extensionto convert the incoming Avro message to Siddhi event. Users should send the pre-defined Avro schema where event conversion will happen with the use of provided avro schema.</p>
...
```
<p style="word-wrap: break-word">Above configuration will do a default Avro input mapping. The input avro message will containing user info will be converted  to a siddhi event.<br>&nbsp;Expected input is a byte array.</p>
